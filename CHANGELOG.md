...
- **Keyboard shortcut** — `Ctrl+Shift+D` toggles dark mode from anywhere in the app.
- **Toast notifications** — Brief slide-up toast confirms theme changes ("Dark mode enabled" / "Light mode enabled").
- **CSS-only tooltips** — Hovering sidebar tab buttons shows descriptive tooltips with no JavaScript required.
